...
export async function exchangeFaydaCodeForToken(code: string) {
  try {
    ...
    if (!res.ok) {
      ...
      console.error("Token exchange failed with status:", res.status, "Error:", errorText)
...
const generateSignedJwt = async (clientId: string) => {
  ...
  try {
    ...
    throw new Error("Invalid PRIVATE_KEY format or content. Please check your environment variable.")
  }
